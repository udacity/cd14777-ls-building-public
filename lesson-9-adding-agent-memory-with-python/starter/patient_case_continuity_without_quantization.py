...
class CustomChatModel(BaseChatModel):
    """A LangChain-compatible wrapper for the local Hugging Face model."""
    model: Any
    tokenizer: Any

    def _generate(self, messages: List[BaseMessage], **kwargs: Any) -> ChatResult:
        # Convert messages to chat format
        formatted_msgs = [{"role": m.type if m.type != "ai" else "assistant", "content": m.content} for m in messages]
